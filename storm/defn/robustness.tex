...
\begin{figure}[p]
  ...
  \begin{pcvstack}
    ...
    \fbox{\algcom $\adv$ wins if any honest party ends in a disjoint state} \\
...
\begin{figure}[p]
  ...
  \footnotesize
  \begingroup
  \renewcommand{\sloppy}{\tolerance 9999\emergencystretch 3em\hfuzz 30pt\vfuzz 30pt}%
  \makeatletter\renewcommand*\@pseudocodecodesize{\scriptsize}\makeatother
  \begin{pchstack}[space=0.5em]
  {
    %\chelsea{TODO: this needs to be script size for the proceedings version}
  }
	\pseudocode{
    \procedure[]{$\oraclekg(\{\allowbreak\{ \pmsg_{i,j}\}_{j \in \honest},\allowbreak \bmsg_{ri} \}_{i \in \corrupt})
    $}{
  ...
  \pchspace
  \begin{pcvstack}
  \procedure[]{$\oraclefinalize(\{\allowbreak\{ \pmsg_{i,j}\}_{j \in \honest},\allowbreak \bmsg_{ri} \}_{i \in \corrupt})$}{
...
When considering simulation-based definitions,
the definition either considers a scheme that is strongly robust,
or where the adversary can abort the protocol at any point during the protocol execution,
even after seeing the output key material.
However, weak robustness allows the adversary to abort the protocol execution,
but only before some critical state;
i.e., before the adversary has learned any information about the output key.
As such, it is a more fine-grained notion of tolerated aborts than a simulation-based notion of abort,
as weak robustness considers where in the protocol execution the adversary is
allowed to abort.
