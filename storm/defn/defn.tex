...
Instead, each party $i$ has a share $\sk_i$ of the secret key,
...
We begin by defining a target single-party key generation scheme $\targetkeygen$
that the DKG implements in a distributed manner.

\begin{definition}
A \defn{target key generation scheme} $\targetkeygen$
is the tuple $\targetkeygen = (\keygen,$\allowbreak$\verify)$, where:
\begin{itemize}
    \item $\keygen(1^\lambda) \rightarrow (\SK, \PK)$:
      A probabilistic algorithm that
      accepts as input a security parameter $\lambda$
      and outputs a secret and public keypair $(\SK, \PK)$,
      where $\SK \in \secretspace$ and $\PK \in \publicspace$.
    \item $\verify(\SK, \PK) \rightarrow \zo$:
      A deterministic algorithm that accepts as input
      a secret and public keypair $(\SK, \PK)$,
...
\begin{definition}
  ...
  \begin{itemize}
    \item $\keygen(1^\lambda, n, t) \rightarrow  \langle (\PK, \qual, \auxinfo), (\status_i,\sk_i)_{i \in \set{n}}\rangle$:
      ...
      \begin{itemize}
        \item $\dkgkeygen_0(1^\lambda, n, t, \id_i) \rightarrow (\party_i,\outvecp,\bmsg_i)$:
    ...
    % CK- yes, but so can any variable, we don't need to say that explicitly here.
        \item $\dkgkeygen_k(\party_i, \invecp, \invecb) \rightarrow (\party_i, \outvecp, \bmsg_i)$:
          ...
          the vector $\invecp$ of any peer-to-peer messages sent by other participants in the previous round,
        ...
        \item {\sloppy $\finalize(\party_i, \invecp, \invecb) \rightarrow$
          $\langle(\PK, \qual,\auxinfo),$\allowbreak$(\status_i, \sk_i)_{i \in \set{n}}\rangle$:
          Accepts participant state $\party_i$,
          peer-to-peer messages $\outvecp$, and broadcast messages $\invecb$.
          Outputs a public output $(\PK, \qual,\auxinfo)$
          and a private output $(\status_i, \SK_i)$.\par}
      \end{itemize}
...
\begin{align*}
    \text{if } & \dkg.\keygen(1^\lambda, n, t) \rightarrow \left((\PK, \qual), \langle (\status_i, \sk_i)\rangle_{i \in \set{n}}\right) \\[1mm]
    \text{then } & \dkg.\recover(\PK, t, \{(i,\SK_i)\}_{i \in \coalition}) \rightarrow \SK \text{ for all }  \coalition \subseteq \set{n} \cap \qual, \lvert \coalition \rvert \geq t, \\[1mm]
