...
As such, the risk exists that Pedersen's DKG might be misused in contexts for which it cannot been proven secure.
...
composable across a range of use cases.
...
\RG is composable with existing threshold cryptosystems such
...
In the table, composable denotes if the construction has a standalone proof of security
and can be used in any context that its target (single-party) key generation
algorithm is used (i.e., the scheme is secure under composition with other schemes).
