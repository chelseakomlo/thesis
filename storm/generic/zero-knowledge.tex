...
\begin{theorem} \label{thm:zero-knowledge}
  The generic construction is zero-knowledge in the honest majority setting ($t < n/2$) in the random oracle model,
  against a PPT adversary $\adv$ controlling up to $t-1$ participants,
  assuming the generic construction itself is weakly robust,
  the \AVSS achieves aggregated secrecy,
  and the \ENIKE is unrecoverable.
%  \ian{Thm 1 specified an honest majority.  Is that not a requirement here as well?}
  ...
  \begin{split}
      \advantzk{\dkg,\adv,\simkeygen_i,\simfin}(\lambda) & \leq \advantsecrecy{\avss[\Hash_1],\bdv_1}(\lambda)\ + \advant{rec}{\enike,\bdv_3}(\lambda) \\
...
\pcvspace
\procedure[linenumbering]{$\dkgkeygen_2(\party_i, \invecp, \invecb)$} {
  \pcfor j \in \set{n}, j \neq i \pcdo \\
...
\end{pchstack}
    \caption{Game 2 for the Zero Knowledge Proof for Our Generic Construction.
    }
  \label{fg:zk-g3}
\end{figure}
...
moved from $\dkgkeygen_3$ to $\dkgkeygen_2$.

Rather than learning corrupted players' blinding factors as input to $\dkgkeygen_3$,
$\cdv$ instead derives blinding factors for all corrupted parties in $\dkgkeygen_2$,
...
$\simid$, $\bdv_4$ instead simulates the protocol via $\simkeygen$;
...
then $\bdv_3$ wins the \ENIKE game with probability $\frac{1}{q}$.
...
\begin{equation} \label{eq:game3-secrecy}
  \bigabs{\Pr[W_3] - \Pr[W_2] } \leq  \advant{unpred}{\enike,\bdv_3}(\lambda) \cdot \frac{1}{q}
\end{equation}
...
receiving $(\{ \share_j \}_{j \in \honest}, \vsscommit^*)$ in return.
...
\begin{equation} \label{eq:game4-zk}
  \bigabs{\Pr[W_4] - \Pr[W_3] } \leq \advant{asec}{\avss, \bdv_4, \simshare}(\lambda)
\end{equation}
