...
%  \ian{Thm 1 specified an honest majority.  Is that not a requirement here as well?}
  ...
  \begin{split}
      \advantzk{\dkg,\adv,\simkeygen_i,\simfin}(\lambda) & \leq \advantsecrecy{\avss[\Hash_1],\bdv_1}(\lambda)\ +
        \advant{rec}{\enike,\bdv_3}(\lambda) \cdot \frac{1}{\lvert \Hash_1 \rvert} \\
  ...
  and $\lvert \Hash_1 \rvert$ denotes the order of the co-domain of $\Hash_1$.
\end{theorem}
...
\begin{equation} \label{eq:game3-secrecy}
  \bigabs{\Pr[W_3] - \Pr[W_2] } \leq  \advant{rec}{\enike,\bdv_3}(\lambda) \cdot \frac{1}{\lvert \Hash_1 \rvert}
\end{equation}
...
\begin{equation} \label{eq:game4-zk}
  ...
  \advant{asec}{\avss, \bdv_4[\Hash_1], \simshare,\simgettweak}(\lambda)
\end{equation}
