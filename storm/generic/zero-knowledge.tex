...
Let $\cdv$ be the experiment that implements the DKG zero-knowledge experiment to $\adv$.
...
$\cdv$ handles $\adv$'s random oracle queries by lazy sampling, as follows:
  ...
  \begin{itemize}
    \item[$\underline{\Hash_1}$]: When the adversary queries $\Hash_1$ on inputs $(\pubaggregateset_i, \auxstring_i)$,
      $\cdv$ checks if it is in $Q_1$, and if so, returns the corresponding $\tweak_i$.
      Otherwise, $\cdv$ randomly samples $\tweak_i$ from its respective domain,
    ...
    \item[$\underline{\Hash_2}$]: When the adversary queries $\Hash_2$ on inputs $\{\blinding_1,\ldots,\blinding_n \}$,
      $\cdv$ checks if it is in $Q_2$, and if so, returns the corresponding $\auxstring_i$.
      Otherwise, $\cdv$ randomly samples $\auxstring_i$ from its respective domain,
...
This however does not impact the ability of $\cdv$ to simulate all honest players,
in part because $\cdv$ never reveals $\{ \SK_i \}_{i \in \honest}$  directly.
...
$\bdv_1$ is constructed similar to Game~0,
...
as corrupt parties will receive valid shares (and so can derive their secret keys correctly),
and honest parties' shares (and secret keys) are never revealed.
...
\end{pchstack}
    ...
    {Game~2 for the Zero Knowledge Proof for Generic Construction.
     The boxes indicate steps that are performed in addition to Game~1.
    }
  \label{fg:zk-g3}
\end{figure}
...
The only difference between Game~3 and Game~2 is that if the adversary
queries the random oracle $\Hash_1$ with respect to any of the honest
parties' blinding values $\{\blinding_i \}_{i \in \honest}$
before being provided with the witness values
$\{ \witness_i\}_{i \in \honest}$ as input to $\dkgkeygen_3$,
...
When simulating $\oracle^{\dkgkeygen_3}$,
...
%
If $\adv$ queries $\Hash_1$ with $\blinding_\simid$ before $\bdv_3$ has completed $\oracle^{\dkgkeygen_2}$,
then $\bdv_3$ wins the \ENIKE game with probability $\frac{1}{q_h}$.
...
From Game~3, we know that $\cdv$ can derive $\auxstring$ in $\oracle^{\dkgkeygen_3}$,
...
When $\adv$ queries $\oracle^{\dkgkeygen_1}$,
for each honest player $k \in \honest$:\linebreak[4]
$\bdv_4$ queries the \AVSS oracle $\getshare()$,
receiving $(\{ \share_j \}_{j \in \honest}, \vsscommit^*)$.
$\bdv_4$ then uses these outputs to simulate participant $\simid$
for $\oracle^{\dkgkeygen_1}$.

$\bdv_4$ follows the protocol honestly for all other players,
and submits all $(\{ \share_{kj} \}_{j \in \honest}, \vsscommit_k)$
for $k \in \set{n} \setminus \simid$
