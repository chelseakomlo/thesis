...
\begin{theorem} \label{thm:perfect-robust}
  The generic construction is weakly robust in the honest majority setting
  ($t-1 < n/2$) against
  ...
  such that Assumption~\ref{ass:avss-enike} holds.
...
as shown in Figure~\ref{fg:enike-binding}.
Recall that in the binding game,
the adversary accepts as input only the public security parameters $\pp$,
and is required to output two pairs $(\sk_1^*, \pk_1^*, \sk_2^*, \pk_2^*)$
such that $\verifykeypair$ outputs $1$ for both pairs,
but $\enike.\getsharedkey(\sk_1^*, \pk_2^*) \neq \enike.\getsharedkey(\sk_2^*, \pk_1^*)$.
...
Recall that the only inputs to $\bdv_1$ from the NIKE binding game are the NIKE public parameters.
...
\noindent where $\share_{j, \ell} = \share_{j, \ell}'$,
except for $\ell = k$.
...
if any honest party aborts in $\dkgkeygen_1$ or $\dkgkeygen_2$,
then all honest parties will likewise abort.
...
While honest parties might abort (meaning strong robustness cannot be met),
honest parties will consistently either end in an aborted or accepting status,
meaning the weak robustness game can be fulfilled.
The only remaining opportunity for $\adv$ to win the weak robustness game
is to disrupt consistency of $\pk$, $\qual$, $\auxinfo$,
