...
We now introduce a weakly robust generic DKG construction that assumes an honest majority,
shown in Figure~\ref{fg:generic-perfect}.
The DKG initialized with a NIKE $\enike$ as defined in Section~\ref{enike},
an \AVSS $\avss$ as defined in Section~\ref{asym-vss},
and a hash function $\Hash$.
We present the concrete construction in Section~\ref{storm-gen}.
Recall that setup functions
$\pp \gets \enike.\Setup(1^\lambda);\ \pp' \gets \avss.\Setup(1^\lambda)$ are performed at setup,
not at the time of key generation.
...
\procedure[]{$\dkgkeygen_0[\Hash](\lambda, n, t, i)$} {
    (\secretone_i, A_i) \rgets \enike.\keygen() \\
    \mathsf{in} \gets (\secretone_i, n, t) \\
    ...
    (\witness_i, \commitment_i) \rgets \enike.\keygen() \\
...
\pcvspace
\procedure[]{$\dkgkeygen_1[\Hash](\party_i, \invecp_1, \invecb_1)$} {
  ...
  \pcind \pcind \party_i.\status = \abort;\ \bmsg_{1,i} \gets \fail \\
...
\pcvspace
\procedure[]{$\dkgkeygen_2[\Hash](\party_i, \emptyset, \invecb_2)$} {
...
\pchspace
\begin{pcvstack}
\procedure[]{$\finalize[\Hash](\party_i, \emptyset, \invecb_3)$} {
  ...
  %\text{\algcom the correct opening $\witness_j$} \\
    ...
    \pcif \corrupt \neq \emptyset \\
    \pcind \textbf{Broadcast } \{ \share_{ji} \}_{j \in \corrupt} \\
    %\text{\algcom Broadcast shares for corrupt} \\
    %\text{\algcom parties to all other participants} \\
    \pcind \textbf{Receive } \{ \share_{jk} \}_{j \in \corrupt, k \in \qual} \\
    \pcind \pcfor k \in \qual \pcdo \\
    \pcind \pcind \pcif \exists\ j \ : \ \avss[\Hash_1].\verify(i, \share_{jk}, \vsscommit_j) \neq 1 \\
    \pcind \pcind \pcind \qual \gets \qual \setminus \{ k\} \\
    \pcfor j \in \corrupt \pcdo \\
    \pcind \secretone_j \gets \avss[\Hash_1] \\
    \pcind \pcind .\recover(t, \{(k,\share_{jk}) \}_{k \in \qual} ) \\
...
\end{pcvstack}
\end{pchstack}
    \caption{A generic DKG protocol. See corresponding text for full description.}
...
\end{itemize}
  \item An aggregatable verifiable secret sharing scheme
    $\avss[\Hash_1]~=~(\Setup,
    \issueshares,\verify,\allowbreak\recover,\allowbreak\derivepshare,
    \aggregatepriv, \aggregatepub,\gettweak)$ that is zero-indexed.
  \item A non-interactive key exchange (NIKE) $\enike~=~(\Setup, \keygen,\verifykeypair,\getsharedkey)$.
\end{itemize}
...
\begin{assumption} \label{ass:avss-enike}
  For $\pp \gets \enike.\Setup(1^\lambda)$ and $\pp' \gets \avss.\Setup(1^\lambda)$ \\
  any $(\sk, \pk) \rgets \enike.\keygen()$,
  ...
  \begin{align} \label{eq:avss-enike-rel}
    \begin{split}
      ...
      (\{ (i, \share_i) \}_{i \in \set{n}}), \vsscommit) \rgets \avss[\Hash_1].\issueshares(\sk, n, t) \text{ and } \\
...
\subsection{Key Generation Protocol}
As shown in Figure~\ref{fg:generic-perfect},
the key generation protocol is completed in three network rounds in the optimistic setting and four network
...
after setting their status to $\abort$.\footnote{Because the fail message $\fail$ is sent on a broadcast channel,
...
Note that even if an adversary at this stage were to broadcast incorrect shares,
because we assume an honest majority of players,
then all honest participants will receive a sufficient number of honest shares to recover successfully,
and can verify the correctness of the recovered secret against the commitment sent in earlier rounds.
They can do so
by checking if $\avss[\Hash_1].\verify(i, \share_{jk}, \vsscommit_j) = 1$
for each identifier $j \in \corrupt, k \in \qual$,
updating $\qual \gets \qual \setminus \{ k\}$ if the check does not hold.
...
The public output at the end of $\finalize$ is $(\pk, \qual, \auxinfo)$.
