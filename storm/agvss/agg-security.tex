...
\begin{figure}[!ht]
  \begin{pchstack}[boxed,center,space=1em]
    \procedure[]{$\AggSecurityExp_{\avss, \adv,\simgettweak}(\lambda)$}{
    b \rgets \zo \\
    ...
    \pcind \pcif \avss.\recover(t, \{\ashare_i \}_{i \in \honest}, \avsscommit) \neq \secret \\
    \pcind \textbf{or } \avss.\derivepshare(0, \avsscommit) \neq \vsschal \\
  ...
  \pcvspace
    \procedure[]{$\getshare() \text{\algcom Performs secret sharing}$}{
      ...
      (\{(j,\share_{\total j})\}_{j \in \set{n}}, \vsscommit_\total) \rgets \avss.\issueshares(\lambda, s_\total, n, t) \\
  ...
  \procedure[]{$\recvshare(\{ (j, \share_{\total j}) \}_{j \in \honest}, \vsscommit_\total)$}{
    ...
    \total \gets \total + 1 \\
	  \pcfor j \in \honest \\
    \pcind \pcif \avss.\verify(j, \share_{\total j}, \vsscommit_\total) \neq 1 \\
...
\[
  \advantaggsecurity{\avss, \adv, \simgettweak}(\lambda) = \bigabs{\Pr[\AggSecrecyExp_{\avss, \adv,\simgettweak}(\lambda) = 1 ] - 1/2 }
\]
