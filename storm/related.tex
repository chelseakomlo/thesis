...
Pedersen-DKG~\cite{Pedersen91dkg} is a two-round protocol among $n$ parties,
such that each party plays the role of the dealer for Feldman Verifiable Secret Sharing (VSS)~\cite{Pedersen91}.
Gennaro et al.~\cite{GennaroJKR07} describe an attack against Pedersen-DKG, which we refer to as the \GJKRAttack attack,
...
While the authors later demonstrate unforgeability of a Schnorr threshold signature scheme with Pedersen-DKG employed for key generation~\cite{GennaroJK02},
this proof of security does \emph{not} extend to other applications.
%
Gurkan et al.~\cite{GurkanJMMST21} show that Pedersen-DKG can be used securely in any setting where the cryptographic scheme is ``rekeyable''.
In other words,
...
we observe that their construction is in fact insecure without this round.
We discuss this attack and present  a secure variant in Appendix~\ref{app:gjkr}.
...
Komlo and Goldberg~\cite{KomloG20} introduce a variant of Pedersen-DKG that we refer to as $\pedpop$,
where the dealer additionally sends a Schnorr proof of possession to prevent rogue-key attacks.
Bellare et al.~\cite{BellareCKMTZ22} present an alternative proof of security for $\pedpop$,
in the context of the FROST threshold signature,
but this proof does not extend to the use of $\pedpop$ in other settings.
...
as is demonstrated by alternative DKGs in the literate
publicly verifiable secret sharing scheme (PVSS)~\cite{Groth21,BonehS2020,CascudoDSV23}.

However,
PVSS schemes require that secret key material be encrypted to receivers' public keys,
thereby requiring a complicated decryption step (because receivers must recover
the field element, not just the corresponding group element as is the case in
hybrid public-key encryption).
For this reason,
we define our construction to explicitly incur an additional network round.
However, our construction could instead be adapted to use a PVSS and there by
only two network rounds.
