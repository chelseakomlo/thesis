...
Gennaro et al.~\cite{GennaroJKR03} present a threshold
...
during signing operations as is done in both of the above schemes~\cite{StinsonR01,GennaroJKR03},
...
Recent work by Damg\aa{}rd et al.~\cite{DamgardJNPO22} defines an efficient
...
Whereas prior works~\cite{DrijversEFKLNS19,Wagner02} established the existence of a sub-exponential time algorithm
for solving the ROS (Random inhomogeneities in a Overdetermined Solvable system of linear equation) problem,
(first introduced by Schnorr~\cite{schnorr06}),
Benhamouda et al.~\cite{BenhamoudaLLOR20} give a sub-exponential time algorithm .
...
respect to an idealized notion of threshold signatures by Canetti et al.~\cite{CanettiGGMP20}.
