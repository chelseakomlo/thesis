...
with respect to $R$
and challenge $c = \HashTwo(R, \pkgroup, m)$,
...
We next demonstrate the static (weak) unforgeability of \glitter,
via Theorem~\ref{thm:unforgeability}.
We believe it is possible to additionally prove that \glitter is strongly unforgeable,
but leave this topic for future work.
...
interacting with $\SIM$ than it does against \glitter directly.
...
Concretely, in the honest majority setting with authenticated channels,
    \begin{align} \label{eq:ddv-dl-arctic}
    \begin{split}
...
%\chelsea{I say this as an assumption in the theorem statement, I'm not sure what adding game hops would add.}
  ...
  Because $\DVRFOnem$ is perfectly verifiable in the honest majority setting,
  ...
  Because $\DVRFOnem$ is perfectly unique in the honest majority setting with authenticated channels,
  then
...
%
  ...
  \begin{align} \label{eq:keygen-sim-arctic}
...
%    hash collisions. But at least it is an over-estimate.}
   ...
   it outputs the tuple $(h_j, h_j', \allowbreak \auxstring, \auxstring')$.
   From these values,
