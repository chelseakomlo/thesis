\section{Security of \glitter}
\label{app:arctic-proofs}
...
Recall that we give the unforgeability game in Figure~\ref{fig:euf-cma-threshold}.
...
\begin{proof}[Of Lemma~\ref{lem:simulation}]
...
%\chelsea{I say this as an assumption in the theorem statement, I'm not sure what adding game hops would add.}
  ...
  and let $\advant{verf}{\bdv_2, \DVRFOnem}$ be the advantage that $\bdv_2$ has in the verifiability experiment against $\DVRFOnem$.
  ...
  \lvert \Pr[W_2] - \Pr[W_1] \rvert \leq  \advant{verf}{\bdv_2, \DVRFOnem} = 0
  \end{equation}
  ...
  $\bdv_3$ then outputs
  $(y, (\coalition, (R_j)_{j \in \coalition \cap \corrupt}), \allowbreak (\coalition', (R_j')_{j \in \coalition' \cap \corrupt}))$
  ...
  \begin{enumerate}
    ...
    \item Use the algorithm $\SIMEval$ as defined by \DVRFOne in Figure~\ref{fig:simeval-shine} to generate
      $(R, \{ (r_i, R_i)\}_{i \in \honest} ) \gets \SIMEval(y, r, \allowbreak \corrupt, n,  \{\skrand_i\}_{i \in \corrupt})$.
...
%      \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item $\bdv_4$ then outputs $z_\partyid$ as its output.
  \end{enumerate}

%
  Finally, when $\adv$ terminates,
  $\bdv_4$ outputs $1$.
  ...
  \begin{align} \label{eq:keygen-sim}
	...
	\end{align}
      ...
      \item To simulate key generation for the randomness keys,
...
\begin{proof}[Of Lemma~\ref{lem:reduction}]
