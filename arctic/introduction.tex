...
In this chapter,
we move to the setting where the minimum number of signers $\minsigners$ is generalized to some $t \leq \minsigners \leq n$,
while still preserving security when up to $t-1$ signers may be corrupted.


As we saw in Chapter~\ref{chp:frost},
...
While the goal of efficient and deterministic multi-party Schnorr signature schemes
is desirable, producing such schemes has proven difficult.
...
satisfying the relation $g^z = R \cdot \pk^c$ for a challenge
$c \gets H(R, \pk,m)$.
...
A key-recovery attack is possible is because the adversary can force honest parties to re-use their nonces,
while corrupted parties introduce fresh randomness by picking non-deterministic
nonces.
Doing so would lead to the group commitment $R$ changing,
which in turn leads to the challenge $c$ changing.
An honest participant's secret key share could then be obtained simply by
solving the linear equation $\sk_i = \frac{z_1 - z_2}{c_1 - c_2}$,
where $z_1, z_2$ are signature shares from two different signing sessions,
and $c_1, c_2$ are the corresponding challenges.
...
\begin{quote}
  \it
  \begin{sloppypar}
Can we design a simple, deterministic, and stateless
two-round threshold Schnorr signature scheme,
...
While the protocol allows honest participants to identify that misbehavior has occurred,
the protocol does not support identifying which participant caused the abort
...
%
A \emph{verifiable pseudorandom secret sharing scheme} is simply an extension
...
We give further information is given in Section~\ref{performance-estimates}.
...
\section{Observations of Honest Majority Assumptions in Practice}
\label{hon-maj-obs}
