...
%Compatibility with existing single-party signature schemes is particularly important in practice,
...
In this work,
we focus our attention on threshold Schnorr signatures;
i.e,
a threshold signature scheme that is compatible with the (single-party) Schnorr signature verification algorithm~\cite{Schnorr91}.
Efficient (two-round) threshold Schnorr signatures exist in the randomized
setting~\cite{KomloG20,BellareCKMTZ22,RuffingRJSS22},
...
%}
  \medskip
	\caption{ Comparison of multi-party deterministic Schnorr signature schemes in the random oracle model (ROM) that support general choices of $n$
  (i.e., we exclude $2$-of-$2$ schemes).
  Here,
  $n$ denotes the number of total possible signers,
  $t$ denotes the corruption threshold.
  %and $\minsigners$ denotes the minimum number of parties required to participate in a signing operation.
  ...
  more information is given in Appendix~\ref{app:performance-estimates}.
  Bandwidth similarly is given with respect to a $256$-bit elliptic curve~\cite{NickRSW20,GarillotKMN21}.
  %\ian{Isn't the ``computation'' column the per-signer cost?  Why does Musig-DN have an ``$n$'' in it?}
  %\chelsea{because it is n-of-n and each signer needs to generate a proof and verify n-1 other proofs.}
  }
