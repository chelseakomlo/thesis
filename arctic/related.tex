...
that uses SNARKs to demonstrate in zero-knowledge that a participant
generated its nonce correctly using a PRF with respect to a pre-established keypair.
...
Then, signers generate a Bulletproof~\cite{BunzBBPWM18} to prove in zero-knowledge
that the nonce was derived with respect to $F$, $\sk$, and some input $H(x)$.
\end{sloppypar}
...
each party must then perform $256t$ field operations and 132,000$t$ AES invocations,
