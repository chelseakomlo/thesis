...
all of whom are equally trusted.
...
We show that pseudorandom secret sharing schemes have a natural extension to the publicly verifiability setting.
Finally,
...
\begin{itemize}
  \item $\setup(1^\usecp)$:
A probabilistic algorithm that accepts as input the security parameter $\secp$,
...
Outputs the commitment to the aggregated pseudorandom secret $\vrfpubout$.
    \item $\recover(t, \minparticipants, \coalition, \{ \vrfprivout_j \}_{ j \in \coalition}) \rightarrow \bot /  (\vrfprivout, \vrfpubout )$:
      ...
      the minimum number of participants $\minparticipants$,
      a coalition $\coalition$ with  $\lvert \coalition \rvert \geq \minparticipants$,
      and a set of shares  $\{ \vrfprivout_j \}_{j \in \coalition}$.
      It outputs $\bot$ if $\lvert \coalition \rvert < \minparticipants$
...
when $\pp \rgets \dvrf.\setup(1^\usecp)$
...
\begin{align} \label{eq:vrf-correct}
  \begin{split}
    \text{ for all } i \in \set{n} \text{ and for all } \coalition \subseteq \set{n}, \lvert \coalition \rvert \geq \minsigners,
    \text{ when } \\
    \dvrf.\generate(i, \SK_i, \vrfin) \rightarrow (\vrfprivout_i, \vrfpubout_i),
    \text{ then } \\
    \dvrf.\verify(t, \minparticipants, \coalition \{ \vrfpubout_j \}_{j \in \coalition}) = 1,
    \text{ and } \\
    \dvrf.\recover(t, \minparticipants, \coalition, (\vrfprivout_j)_{j \in \coalition})
    \rightarrow (\vrfprivout, \vrfpubout), \text{ and} \\
    \dvrf.\combinepub(t, \minparticipants, \coalition, (\vrfpubout_j)_{j \in \coalition})  \rightarrow \vrfpubout
  \end{split}
\end{align}
...
\begin{figure}[t]
  \centering
	...
	\begin{pchstack}[boxed]
		\begin{pcvstack}
		\procedure{ $\UniqExp_{\dvrf,\adv}(\secp, n, t, \minparticipants)$ }{
      \pp \rgets \dvrf.\setup(1^\usecp) \\
...
\[
  \advant{uq}{\dvrf,\adv}(\secp, n, t, \minparticipants)
  = \Pr[ \UniqExp_{\dvrf,\adv}(\secp, n, t, \minparticipants)= 1]
\]


  A VPSS $\dvrf$ is \defn{unique} if for all PPT adversaries $\adv$,
  $\advant{unq}{\dvrf,\adv}$ is a negligible function of $\secp$,
...
\begin{figure}[t]
  \centering
	\begin{pchstack}[boxed]
		\begin{pcvstack}
		\procedure{ $\VerifExp_{\dvrf,\adv}(\secp, n, t, \minparticipants)$ }{
      \pp \rgets \dvrf.\setup(1^\usecp) \\
      (\corrupt, \advstate) \rgets \adv(\pp, n, t, \minparticipants) \\
      \pcreturn \bot\ \pcif \lvert \corrupt \rvert \geq t \\
      \honest \gets \set{n} \setminus \corrupt \\
...
Note the environment enforces the authenticity of participant contributions.
...
\[
  \advant{verf}{\dvrf,\adv}(\secp, n, t, \minparticipants)
  = \Pr[ \VerifExp_{\dvrf,\adv}(\secp, n, t, \minparticipants)= 1]
\]

  A VPSS $\dvrf$ is \defn{verifiable} if for all PPT adversaries $\adv$,
  $\advant{verf}{\dvrf,\adv}$ is a negligible function of $\secp$,
  for $n, t, \minparticipants \in \mathbb{N}$,
  $t \leq \minparticipants \leq n$.
\end{definition}
...
\begin{figure}[t]
  \centering
	\begin{pchstack}[boxed]
		\begin{pcvstack}
		\procedure{ $\PseudoExp_{\dvrf,\adv}(\secp, n, t, \minparticipants)$ }{
     b \rgets \zo \\
     z \rgets \zo \\
...
\[
  \advant{psdr}{\dvrf,\adv}(\secp, n, t, \minparticipants)
  = \bigabs{\Pr[ \PseudoExp_{\dvrf,\adv}(\secp, n, t, \minparticipants) = 1]  - 1/2}
\]

  A VPSS $\dvrf$ is \defn{pseudorandom} if for all PPT adversaries $\adv$,
  $\advant{psdr}{\dvrf,\adv}$ is a negligible function of $\secp$,
  for $n, t, \minparticipants \in \mathbb{N}$,
  $t \leq \minparticipants \leq n$.
\end{definition}
