\subsection{Deterministic Threshold Signature Schemes}
...
\begin{itemize}
  ...
  \item $\setup(1^\lambda) \rightarrow \pp$: Accepts as input a security
...
\begin{remark}[Deferring the Choice of Coalition to Later Rounds.]
  Our definition likewise assumes that the coalition of signers $\coalition$ is provided in the first round of signing $\Sign_1$.
...
all allowable $1 \leq \thresh \leq \minsigners \leq n$,
...
\begin{figure}[t]
  \centering
	...
	\begin{pchstack}[boxed]
		\begin{pcvstack}
		\procedure{ $\UnforgExp_{\adv,\dthreshsig}(\lambda, n, t,\minsigners)$ }{
			\pp \leftarrow \dthreshsig.\setup(1^\lambda) \\
	...
	\end{pchstack}
	%\end{linedgamespec}
\caption{Unforgeability game for a deterministic threshold signature scheme $\dthreshsig$ with a $\numrounds$-round signing protocol.
...
%
We present a game-based definition of unforgeability for a deterministic threshold signature scheme in Figure~\ref{fig:euf-cma-threshold}.
...
If the set of corrupt parties is less than the corruption threshold $t$,
...
However,
unlike in the randomized setting,
...
Let the advantage of a static adversary $\adv$ playing the unforgeability game against a deterministic threshold signature scheme $\dthreshsig$ as defined in Figure~\ref{fig:euf-cma-threshold} be as follows:

\[
  \advant{uf}{\adv, \dthreshsig}(\lambda, n, \thresh,\minsigners) = \bigabs{\Pr[ \UnforgExp_{\adv,\dthreshsig}(\lambda,n,t,\minsigners)= 1]}
\]
%
A deterministic threshold signature scheme $\dthreshsig$ is \emph{unforgeable} if for all PPT adversaries $\adv$,
$\advant{uf}{\adv, \dthreshsig}(\lambda, n, t,\minsigners) $ is negligible
in $\lambda$,
for all allowable $n, t,\minsigners \in \mathbb{N}$.
\end{definition}
