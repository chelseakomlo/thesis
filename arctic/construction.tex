...
\begin{pcvstack}
\procedure[linenumbering]{$\setup(\usecp)$} {
...
\pcvspace
\procedure[linenumbering]{$\keygen(n,t,\minsigners) $ } {
...
\pcvspace
  \procedure[linenumbering]{$\sign_1(\partyid, \skparty_\partyid, \msg)$} {
...
\end{pcvstack}
  \pchspace
\begin{pcvstack}
\procedure[linenumbering]{$\sign_2(\partyid, \skparty_\partyid, \msg, \coalition, \{(y_j, R_{j} )\}_{j \in \coalition})$} {
  ...
  c \gets \HashTwo(R, \pkgroup, \msg) \\
  z_\partyid \gets r_\partyid + c \cdot \sksign_\partyid \\
  \pcreturn z_\partyid
}
\pcvspace
  \procedure[linenumbering]{$\combine(\pkgroup, \msg, \coalition, \{(y_j, R_{j}), z_j \}_{j \in \coalition}))$} {
  \mathsf{input} \gets (t, \minsigners, \coalition, \{ R_j\}_{j \in \coalition}) \\
  R \gets \DVRFOnem[\HashOne].\combinepub(\mathsf{input}) \\
  c \gets \HashTwo(R, \pkgroup, \msg) \\
...
\end{pchstack}
    \caption{
      ...
      For security,
      \glitter requires that the minimum number of signing parties
      $\minparticipants \leq n$ be at minimum $\minparticipants \geq 2t-1$,
      where $t$ is the tolerated corruption threshold.
      ...
      %\ian{Is the same $\pp$ meant to be used for both \glitter and \DVRFOne? They don't appear to have the same parameter format?}
      %\chelsea{Yes, but I think it is fine; this falls under the "you know what
      %we mean" category and making it explicit makes it harder to read}
      %\ian{The $\pk_i$ are never used in this protocol.  Do we need to keep them? They \emph{could} be used to check for corrupt players that give incorrect $z_k$ after giving correct $R_k$.}
      %\chelsea{yes, I think we should keep them, for possible extensions}
    }
...
Then, the participant checks its identifier is in the coalition $C$ and that $R_\partyid'$ is in the set of commitments
$(R_j)_{j \in \coalition}$,
...
and the challenge $c \gets \HashTwo(R, \pkgroup, \msg)$.
Finally,
...
which can be verified using the single-party Schnorr verification algorithm given in Definition~\ref{def:schnorr}.
...
%
We give further details on how robustness can be achieved in Appendix~\ref{app:robust-shine}.
%
In this setting,
...
with respect to $R$
and challenge $c = \HashTwo(\pkgroup, R, m)$,
...
\begin{theorem}\label{thm:unforgeability}
	\glitter is unforgeable against a PPT adversary $\adv$ playing the static unforgeability game as shown in Figure~\ref{fig:euf-cma-threshold} against \glitter,
  assuming $\adv$ can make up to $\tcor$ corruptions,
  the number of honest parties is at least $t$,
  the discrete logarithm assumption holds in the ROM,
  ...
  \begin{equation*} \label{eq:glitter-adv}
    \advant{uf}{\adv,\glitterm}(\lambda, n, t, \minsigners) \leq
...
We give the corresponding proof for Theorem~\ref{thm:unforgeability} in Appendix~\ref{app:arctic-proofs}.
