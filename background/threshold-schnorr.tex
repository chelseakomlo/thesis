...
A threshold signature scheme is \emph{correct} if for all security parameters $\secp$, all $1 \leq \thresh \leq n$,
...
the $\Recover$ algorithm outputs $\sk$ and the experiment in Equation~\ref{eqn:protocol-messages} returns signature shares that, when combined with all other shares, produce a signature $\sigma$
...
inspired by the definitions in \cite{CritesKM23,LibertJY16}.
