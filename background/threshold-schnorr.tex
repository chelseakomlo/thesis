...
Portions of this section are adapted from previous work by myself and co-authors~\cite{CritesKM2023}.
...
participants exchange protocol messages $ \gmsg_{ i, j}$,
where $i$ represents the $j$\supth signing round,
and $j$ represents participant $j$.
...
A threshold signature scheme is \emph{correct} if for all security parameters $\pp \gets \Setup(1^\secp)$,
...
the signing algorithms as defined in Equation~\ref{eqn:protocol-messages} returns signature shares that,
...
\begin{figure}[!htb]
    ...
    \pcvspace
		\procedure{$\BigO^{\Sign_\numrounds}(\ssid, k, \PM_{\numrounds-1})$}{
			\pcreturn \bot~\pcif \ssid \notin \sessions \\
      ~~~\vee k \notin \honest \cap \hat{coalition}[\ssid] \vee \round[\ssid, k] \neq \numrounds-1 \\
		...
		\end{pcvstack}
	\end{pchstack}
\caption[Static unforgeability games for a threshold signature scheme.]
{
Static unforgeability games for a threshold signature scheme with $\numrounds$ signing rounds.
}
...
or stronger security models~\cite{CritesKM2023,BachoL22}.
For this reason,
...
and initializes the adversary with $(\pp, n, \thresh)$.
...
$\TUnforgExp_{\A,\threshsig}(\secp, n, \thresh, \tcornew)$, as defined in Figure~\ref{fig:euf-cma-threshold}, be as follows:
%
\[
  \advant{$\tuf$}{\threshsig, \adv}(\secp, n, \thresh, \tcornew) =
  \bigabs{\Pr[\UnforgExp_{\threshsig,\adv}(\secp, n, \thresh, \tcornew) = 1]}
\]
