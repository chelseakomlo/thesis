...
Portions of this chapter are adapted from previous work by Crites et al.~\cite{TODO}.
...
signers and the set $\{ \aggpk_i \}_{i \in \set{n}}$ of public keys for each signer,
  ...
  \item $(\Sign_1, \ldots, \Sign_\numrounds) \rightarrow \{\msg_{ 1, i}, \ldots, \msg_{\numrounds, i} \}_{i \in \S}$: A set of probabilistic algorithms where each subsequent algorithm represents a single stage in an interactive signing protocol, performed by each signing party in a signing set $\S \subseteq \set{n}, |\S| \geq \thresh$, with respect to a message $m$, defined as follows:
  \begin{align}\label{eqn:protocol-messages}
    \begin{split}
    (\msg_{1,i}, \state_{1,i}) &\gets \Sign_1(i, \sk_i, \S, m)   \\
  ...
  $\state_{j,i}$ is the state of signing party $i$ in $\S$ in round $j$.
  The signing set $\S$ and message $m$ are shown here as inputs to the first round,
  but they may be deferred to subsequent rounds,
  depending on the scheme.

\item $\Combine(\S, m, \PM_1, \PM_2, \dots, \PM_\numrounds) \rightarrow (m, \sigma)$:
  A deterministic algorithm that takes as input the signing set $\S$, the message $m$, and a set of protocol messages
  ...
  \item $\Recover(\thresh, \{ (i, \sk_i)\}_{ i \in \S}) \rightarrow \bot / \sk$:
    A deterministic algorithm that takes as input a threshold $\thresh$ and a set of secret key shares $\{(i, \sk_i) \}_{i \in \S}$.
    If $\S \not\subseteq \set{n}$ or $\lvert \S \rvert < \thresh$,
    it outputs $\bot$; else, it recovers $\sk$ using the set of shares.
\end{itemize}
\end{definition}

\begin{remark}
The syntax assumes that the signing set $\S$, message $m$, and secret key share $\sk_i$ are given as input in the first round of signing.
...
all $\S \subseteq [n]$ such that $| \S | \geq \thresh$,
all messages $m$ and
...
\begin{figure}[!htb]
	\begin{pchstack}[boxed]
		\begin{pcvstack}
      \procedure{$\main$ $\G^{\scriptsize{\protect\fbox{$\mathsf{adp}$}\hbox{-}\suf}}_{\A,\threshsig}(\secp, n, \tcor, \tcornew)$}{
		%\protect\dashbox[3.5cm][c]{$\G^{\aduf}_{\A,\threshsig}(\secp, \hmulp)$}}{
			...
			\pp \leftarrow  \ParGen(\usecp) \\
 	    		...
 	    		(\corrupt, \state_\A) \randpick \A(\pp, n, \tcor, \tcornew)   \\
      ...
      ~~~\wedge  \Verify(\threshpk, m^*, \sig^*) = 1\\
   		...
   		\pcvspace
		\procedure{$\Init(\ssid, k, \S, m)$}{
		\pcif \ssid \notin \sessions \\
		~~~\pcreturn \bot~\pcif \S \not\subseteq [n] \vee |\S| \neq \thresh\\
		~~~~\vee k \notin \honest \cap \S \\
		~~~\sessions \gets \sessions \cup \{\ssid\} \\
		~~~\S \gets \S[\ssid];~m \gets m[\ssid] \\
		~~~Q_m \gets Q_m \cup \{m[\ssid]\} \\
		\pcreturn \bot~\pcif k \notin \honest \cap \S[\ssid] \\
		~~~\vee \round[\ssid, k] \neq \bot
		}
		\pcvspace
    		\fbox{ \procedure{$\BigO^{\Corrupt}(k)$}{
       			\pcreturn \bot~\pcif k \not\in \honest \\
            ~~~\vee |\corrupt| > \hmulp \cdot \tcornew \\
        			\corrupt \gets \corrupt \cup \{ k \} \\
			\honest \gets \honest~\backslash~\{ k \} \\
        			\pcreturn (\sk_k, \{ \state[\ssid', k] \}_{\ssid' \in \sessions})
			} }
		\end{pcvstack}
    		\pchspace
		\begin{pcvstack}
			\procedure{$\BigO^{\Sign_1}(\ssid, k, \S, m)$}{
       	 			%\pccomment{$k$ denotes an honest participant identifier} \\
				\pcreturn \bot~\pcif \bot \gets \Init(\ssid, k, \S, m) \\
				%\S \gets \signers[\ssid];~m \gets m[\ssid] \\
				(\PM_1[\ssid, k], \state_1[\ssid, k]) \\
				~~~~~~~~~~~\gets  \Sign_1(k, \sk_k, \S[\ssid], m[\ssid]) \\
        ...
        \pcreturn \PM_{1, k}
			...
			~~~\vee k \notin \honest \cap \S[\ssid] \vee \round[\ssid, k] \neq 1 \\
			\pcparse \{\PM_{1,i}\}_{i \in \S} \gets \PM_1 \\
		...
		\procedure{$\BigO^{\Sign_\numrounds}(\ssid, k, \PM_{\numrounds-1})$}{
			\pcreturn \bot~\pcif \ssid \notin \sessions \\
			~~~\vee k \notin \honest \cap \S[\ssid] \vee \round[\ssid, k] \neq \numrounds-1 \\
			\pcparse \{\PM_{\numrounds-1,i}\}_{i \in \S} \gets \PM_{\numrounds-1} \\
		...
		\end{pcvstack}
	\end{pchstack}
\caption{Static and adaptive unforgeability games for a threshold signature scheme with $\numrounds$ signing rounds.  The public parameters $\pp$ are implicitly given as input to all algorithms, and $\PM_1, \PM_2, \dots, \PM_\numrounds$ represent protocol messages defined within the construction.
The static game contains all but the boxes, and the adaptive game adds the boxes.
...
In this work, we are interested in the notion of \emph{adaptive} security.
We provide a game-based definition of adaptive security in \ref{fig:euf-cma-threshold},
...
Let the advantage of an adversary $\A$ playing the adaptive security game
$\G^{\aduf}_{\A,\threshsig}(\secp, n, \tcor, \tcornew)$, as defined in Figure~\ref{fig:euf-cma-threshold}, be as follows:
%
\[
  \advant{adp-sec}{\A,\threshsig}(\secp, n, \tcor, \tcornew) = \bigabs{\Pr[\G^{\aduf}_{\A,\threshsig}(\secp, n, \tcor, \tcornew) = 1]}
\]

A threshold signature scheme $\threshsig$ is \emph{adaptively secure} if for all PPT adversaries $\A$,
$\advant{adp-sec}{\A}(\secp, n, \tcor, \tcornew) $ is negligible,
for $(n, \tcor, \tcornew)  \in \mathbb{N}, \tcor \leq  n, \tcornew < \tcor$.
\end{definition}

\begin{definition}[Full Adaptive Security]
We say that a threshold signature scheme $\threshsig$ achieves \emph{full adaptive security} if the
corruption threshold $\tcornew$ in $\G^{\aduf}_{\A,\threshsig}(\secp, n, \tcor, \tcornew)$ is equal to the reconstruction threshold;
i.e., $\tcornew = \tcor$.
\end{definition}
