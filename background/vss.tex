...
Zero-indexing is important later in our generic construction,
...
\begin{figure}[t]
  ...
  \begin{pcvstack}
    \procedure[linenumbering]{$\SecrecyExp_{\vess, \adv, \simshare}(\lambda,\secret)$}{
    b \rgets \zo \\
    (n, t, \corrupt, \advstate) \rgets \adv(\lambda) \\
    \pcreturn 0\ \pcif t > n \textbf{ or } \corrupt \not\subseteq \set{n} \textbf{ or } \lvert \corrupt \rvert \geq t  \\
    %
    \pcif b=0 \\
    \pcind (\{(j, \share_j)\}_{j \in \set{n}}, \vsscommit) \gets \vess.\issueshares(\lambda, \secret, n, t) \\
    \pcelse \text{\algcom $b=1$ case} \\
    \pcind \vsschal \rgets \vess.\vfunc(\secret) \\
    ...
    \pcind \text{\algcom The simulation must produce $\lvert \corrupt \rvert$ valid shares} \\
    \pcind \pcreturn 1\ \pcif \vess.\derivepshare(0, \vsscommit) \neq \vsschal  \\
    \pcind \text{\algcom The commitment must be valid with respect to $\vsschal$} \\
...
\begin{figure}[!ht]
  \begin{pchstack}[boxed,center,space=1em]
  \begin{pcvstack}
    \procedure[linenumbering]{$\UniquenessExp_{\vess,\adv}(\lambda)$}{
    (t^*, \recoveryset_1^*, \recoveryset_2^*, \vsscommit^*) \rgets \adv(\lambda) \\
    \text{\algcom $\adv$ chooses two recovery sets and a commitment} \\
    \text{\algcom with respect to the same threshold} \\
...
$\secretspace$.
Importantly, we do not restrict secrets to be sampled uniformly at random; even degenerate cases should hold.
