...
\begin{figure}[t]
	...
	\begin{pchstack}[boxed,space=1em]
		\begin{pcvstack}[space=1em]
			\procedure{$\\textsf{Exp}^{\eufcma}_{\A,\dsig}(\lambda)$}{
...
A digital signature scheme $(\Setup, \KeyGen, \Sign, \allowbreak \Verify)$ is \emph{correct} if for all security parameters $\secp \in \mathbb{N}$,
...
The Schnorr signature scheme consists of polynomial-time algorithms $(\Setup, \KeyGen, \Sign, \Verify)$, defined as follows:
  ...
  \begin{itemize}
    ...
    \item $\Setup(1^\secp) \rightarrow \pp$:
    On input a security parameter $1^\secp$,
    run $(\Gr, q, g) \gets \GroupGen(\usecp)$ and select a hash function $\Hash: \{0,1\}^* \rightarrow \Z_p$.
