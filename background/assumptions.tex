...
\begin{pchstack}
  \procedure{$\main$ $\textsf{Exp}^{\dl}_{\A}(\secp)$}{
...
\begin{pchstack}
  \vline
      \pchspace
  \procedure{$\main$ $\textsf{Exp}^{\ell\hbox{-}\protect\fbox{$\mathsf{a}$}\omdl}_{\A}(\secp)$ }{
  (\Gr, p , g) \randpick \GroupGen(\usecp) \\
  Q \gets \emptyset \\
  q \gets 0 \\
  ...
  \pchspace
  \procedure{$\BigO^{\dl}(X, \protect\fbox{$\alpha, \{\beta_i\}_{i=0}^{\ell}$} )$\  }{
    \pccomment{\protect\fbox{$X = g^{\alpha} \prod_{i=0}^{\ell} X_i^{\beta_i}$}} \\
  q \gets q + 1 \\
